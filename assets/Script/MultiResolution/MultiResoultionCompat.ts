import { _decorator, Component, ResolutionPolicy, UITransform, v2, Vec2, View, view } from 'cc';
...
@ccclass('MultiResoultionCompat')
export default class MultiResolutionCompat extends Component {
    ...
    onLoad() {
        ...
        // var proportion = canvasSize.getComponent(UITransform).width / canvasSize.getComponent(UITransform).height;
        // if (proportion >= 0.6) {
        ...
        // let screenResolution = canvasSize.getComponent(UITransform).width / canvasSize.getComponent(UITransform).height;
        ...
        //     cc.log(`屏幕分辨率（${canvasSize.getComponent(UITransform).width}x${canvasSize.getComponent(UITransform).height}) 宽高比：${screenResolution}`);
        //     if (designResolution > screenResolution) {
        //         cc.log(
        //             `设计分辨率缩放至于屏幕分辨率等高后的分辨率 (${designResolutionWidth * (canvasSize.getComponent(UITransform).height / designResolutionHeight)}x${
        //                 canvasSize.getComponent(UITransform).height
        //             })`
        //         );
        //         cc.log("设计分辨率宽高比大于屏幕分辨率，按照官方说法，此时应该采用(Fit Height)，实际应用后，会裁剪掉游戏左右两边的部分内容");
        //     } else if (designResolution < screenResolution) {
        //         cc.log(
        //             `设计分辨率缩放至于屏幕分辨率等宽后的分辨率 (${canvasSize.getComponent(UITransform).width}x${designResolutionHeight *
        //                 (canvasSize.getComponent(UITransform).width / designResolutionWidth)})`
        //         );
        ...
        // }
        // if (cc.view.getCanvasSize().getComponent(UITransform).width > cc.view.getCanvasSize().getComponent(UITransform).height) {
        //     this.backgroundNode.scale = cc.view.getCanvasSize().getComponent(UITransform).width / 720;
        // } else {
        //     this.backgroundNode.scale = cc.view.getCanvasSize().getComponent(UITransform).height / 1280;
        // }
        // if (CC_DEBUG) {
        //     cc.log("调整后");
        //     cc.log(`节点宽高: ${this.node.getComponent(UITransform).width} x ${this.node.getComponent(UITransform).height}`);
    ...
    static getShowAllModeScale(): number {
        return Math.min(
            view.getVisibleSize().width / this.DEFAULT_RESOLUTION_WIDTH,
            view.getVisibleSize().height / this.DEFAULT_RESOLUTION_HEIGHT
        );
    }
    //    /**
    ...
    static getShowAllModeVerticalBorderHeight(): number {
        return view.getVisibleSize().height - this.getShowAllModeRealHeight();
        //  view.getCanvasSize().height - this.getShowAllModeRealHeight();
    }
    //    /**
    //     * 计算当前游戏设计分辨率在ShowAll模式适配后，左右边界的黑边之和在实际画布中的长度
    //     */
    static getShowAllModeHorizontalBorderWidth(): number {
        return view.getCanvasSize().width - this.getShowAllModeRealWidth();
    }
    //    /**
    //     * 计算当前游戏设计分辨率在ShowAll模式适配后，传入来的原始坐标在ShowAll模式下的「贴近屏幕底部」实际坐标值
    //     */
    static getShowAllModeNodePositionCloseToBottom(nodePosInDesign: Vec2): Vec2 {
        ...
        return v2(nodePosInDesign.x, nodePosYInDesign);
    }
    //    /**
    //     * 将节点在设计分辨率中的坐标转换为节点在实际画布中的坐标
    //     */
    static convertNodePosInDesignToNodePosInCanvas(nodePosInDesign: Vec2) {
        return nodePosInDesign.subtract(v2(this.getShowAllModeScale(), this.getShowAllModeScale()));
    }
    //    /**
    //     * 将节点在实际画布中的坐标转换为节点在设计分辨率中的坐标
    //     */
    static convertNodePosInCanvasToNodePosInDesign(nodePosInCanvas: Vec2) {
        return nodePosInCanvas.divide(v2(this.getShowAllModeScale(), this.getShowAllModeScale()));
    }
    //    /**
...
//     static getShowAllModeScale(): number {
//         return Math.min(
//             cc.view.getCanvasSize().getComponent(UITransform).width / this.DEFAULT_RESOLUTION_WIDTH,
//             cc.view.getCanvasSize().getComponent(UITransform).height / this.DEFAULT_RESOLUTION_HEIGHT
//         );
//     }
// 
//     /**
...
//      */
//     static getShowAllModeVerticalBorderHeight(): number {
//         return cc.view.getCanvasSize().getComponent(UITransform).height - this.getShowAllModeRealHeight();
//     }
// 
//     /**
//      * 计算当前游戏设计分辨率在ShowAll模式适配后，左右边界的黑边之和在实际画布中的长度
//      */
//     static getShowAllModeHorizontalBorderWidth(): number {
//         return cc.view.getCanvasSize().getComponent(UITransform).width - this.getShowAllModeRealWidth();
//     }
// 
//     /**
//      * 计算当前游戏设计分辨率在ShowAll模式适配后，传入来的原始坐标在ShowAll模式下的「贴近屏幕底部」实际坐标值
